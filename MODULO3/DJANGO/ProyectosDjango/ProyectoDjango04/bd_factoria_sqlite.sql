-- 1. SQLite doesn't use DROP DATABASE, we'll just attach to a new database file
-- 2. In SQLite, databases are files - we'll just proceed with table creation

-- 3. SQLite doesn't need USE statement, we're already working with the database
...
-- 5. MOSTRAR LAS TABLAS DE UNA BASE DE DATOS (SQLite alternative)
SELECT name FROM sqlite_master WHERE type='table';

-- 6. MOSTRAR LA ESTRUCTURA DE UNA TABLA (SQLite alternative)
